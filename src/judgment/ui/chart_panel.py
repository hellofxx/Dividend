...
class ChartPanel:
    ...
    def __init__(self, parent):
        ...
        self.chart_frames = []
        for i in range(6):
            chart_frame = ttk.LabelFrame(self.frame, text=f"图表{i+1}")
            chart_frame.pack(fill=tk.BOTH, expand=True, pady=5, padx=5)
            self.chart_frames.append(chart_frame)
        
        # 图表实例
        self.charts = {
            'kline': KlineChart(figsize=(10, 4)),
            'volume': VolumeChart(figsize=(10, 2)),
            'macd': MACDChart(figsize=(10, 2)),
            'rsi': RSChart(figsize=(10, 2)),
            'kdj': KDJChart(figsize=(10, 2)),
            'bollinger': BollingerChart(figsize=(10, 2))
        }
    ...
    def show_error(self, error_msg: str):
        ...
        for i, frame in enumerate(self.chart_frames):
            self.loading_labels[i].pack_forget()
    ...
    def update_charts(self, df: pd.DataFrame):
        ...
        # 更新成交量图
        self._update_chart(1, 'volume', df)
        
        # 更新MACD图
        self._update_chart(2, 'macd', df)
        
        # 更新RSI图
        self._update_chart(3, 'rsi', df)
        
        # 更新KDJ图
        self._update_chart(4, 'kdj', df)
        
        # 更新布林带图
        self._update_chart(5, 'bollinger', df)
